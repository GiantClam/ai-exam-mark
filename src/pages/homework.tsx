...
const Homework: React.FC = () => {
  ...
  const handleViewPdf = (studentIndex: number) => {
    ...
    if (student && student.pdfUrl) {
      ...
      // 添加环境变量前缀，确保使用完整URL
      if (pdfUrl.startsWith('/') && !pdfUrl.startsWith('http') && !pdfUrl.startsWith('https')) {
        pdfUrl = `${process.env.NEXT_PUBLIC_API_URL || ''}${pdfUrl}`;
        console.log(`[PDF] Added environment API prefix to URL: ${pdfUrl}`);
      }
